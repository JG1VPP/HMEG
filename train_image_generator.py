...
class Trainer(object):
    ...
    def __init__(self, args, device):
        ...
        self.obj_discriminators, self.d_obj_kwargs = self._build_obj_discriminators(
            self.vocab
        )
        self.img_discriminators, self.d_img_kwargs = self._build_img_discriminators(
            self.vocab
        )
        ...
        if self.obj_discriminators is not None:
            ...
            self.optimizer_d_obj = torch.optim.Adam(
                itertools.chain(*[d.parameters() for d in self.obj_discriminators]),
        ...
        if self.img_discriminators is not None:
            ...
            self.optimizer_d_img = torch.optim.Adam(
                itertools.chain(*[d.parameters() for d in self.img_discriminators]),
                lr=args.learning_rate,
            )
        ...
        for epoch in range(self.args.num_epochs):
            for t, batch in enumerate(tqdm(self.train_loader, desc=f"epoch {epoch}")):
                ...
                if step == self.args.eval_mode_after:
                    ...
                    self.generator.eval()
                    self.optimizer = torch.optim.Adam(
                        self.generator.parameters(), lr=self.args.learning_rate
                    )
                ...
                if self.obj_discriminators is not None:
                    for i in range(len(imgs)):
                        scores_fake, ac_loss = self.obj_discriminators[i](
                            imgs_pred[i], objs, boxes, obj_to_img
                        )
                        total_loss = add_loss(
                            total_loss,
                            ac_loss,
                        ...
                        weight = (
                            self.args.discriminator_loss_weight * self.args.d_obj_weight
                        )
                        total_loss = add_loss(
                            total_loss,
                            self.g_loss_func(scores_fake),
                            losses,
                            "g_gan_obj_loss_%d" % i,
                            weight,
                        )
                if self.img_discriminators is not None:
                    for i in range(len(imgs)):
                        ...
                        weight = (
                            self.args.discriminator_loss_weight * self.args.d_img_weight
                        )
                        total_loss = add_loss(
                            total_loss,
                            self.g_loss_func(scores_fake),
                ...
                if self.obj_discriminators is not None:
                    ...
                    for i in range(len(imgs)):
                        ...
                        scores_fake, ac_loss_fake = self.obj_discriminators[i](
                            imgs_fake, objs, boxes, obj_to_img
                        )
                        scores_real, ac_loss_real = self.obj_discriminators[i](
                            imgs[i], objs, boxes, obj_to_img
                        )
    ...
    def _check(self, epoch, t):
        ...
        if self.obj_discriminators is not None:
            self.checkpoint["d_obj_state"] = [
                d.state_dict() for d in self.obj_discriminators
            ]
            self.checkpoint["d_obj_optim_state"] = self.optimizer_d_obj.state_dict()
        if self.img_discriminators is not None:
            self.checkpoint["d_img_state"] = [
                d.state_dict() for d in self.img_discriminators
            ]
        ...
        checkpoint_path = os.path.join(
            self.args.output_dir, "%s_with_model.pt" % self.args.checkpoint_name
        )
        print("Saving checkpoint to ", checkpoint_path)
        torch.save(self.checkpoint, checkpoint_path)
        checkpoint_path = os.path.join(
            self.args.output_dir, "%s_no_model.pt" % self.args.checkpoint_name
        )
        key_blacklist = [
            "model_state",
            "optim_state",
    ...
    def _check_model(self, loader):
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                model_masks = masks
                model_out = self.generator(
                    objs, triples, obj_to_img, boxes_gt=boxes, masks_gt=model_masks
                )
                (
                    imgs_64_pred,
                    imgs_128_pred,
            ...
            model_out = self.generator(
                objs, triples, obj_to_img, boxes_gt=boxes, masks_gt=masks
            )
    ...
    def _build_crohme_dsets(self):
        ...
        train_dset = CROHMELabelGraphDataset(
            CROHME_DIR, names[:-500], nc=nc, image_size=self.args.image_size
        )
    ...
    def load(self, restore_path):
        ...
        for i in range(3):
            self.obj_discriminators[i].load_state_dict(
                self.checkpoint["d_obj_state"][i]
            )
        self.optimizer_d_obj.load_state_dict(self.checkpoint["d_obj_optim_state"])
        for i in range(3):
            self.img_discriminators[i].load_state_dict(
                self.checkpoint["d_img_state"][i]
            )
...
def calculate_model_losses(
    ...
    if skip_pixel_loss:
        l1_pixel_weight = 0
    l1_pixel_loss = (
        F.l1_loss(img_preds[0], imgs[0])
        + F.l1_loss(img_preds[1], imgs[1])
        + F.l1_loss(img_preds[2], imgs[2])
    )
    total_loss = add_loss(
        total_loss, l1_pixel_loss, losses, "L1_pixel_loss", l1_pixel_weight
    )
    loss_bbox = F.mse_loss(bbox_pred, bbox)
    total_loss = add_loss(
        total_loss, loss_bbox, losses, "bbox_pred", args.bbox_pred_loss_weight
    )
    ...
    if args.mask_loss_weight > 0 and masks is not None and masks_pred is not None:
        ...
        total_loss = add_loss(
            total_loss, mask_loss, losses, "mask_loss", args.mask_loss_weight
        )
